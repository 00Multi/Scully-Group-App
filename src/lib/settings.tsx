...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<SettingsCtx>(() => {
    ...
    // Apply a new schema and record the change so it can be undone/redone. Undo
    // and redo set the exact before/after snapshots directly (bypassing this
    // helper), so replaying them never records a fresh entry.
    const commit = (next: Schema, label: string) => {
      const prev = schema;
      if (next === prev) return;
      setSchema(next);
      record({
    ...
    const addField: SettingsCtx["addField"] = (groupId, input = {}) => {
      ...
      const newKey = uniqueKey(slugify(label), taken);
      ...
      commit({ ...schema, fields: [...schema.fields, field] }, `Add data point “${label}”`);
      return newKey;
    };

    const addImportedFields: SettingsCtx["addImportedFields"] = (incoming) => {
      if (!incoming.length) return;
      const haveKeys = new Set(schema.fields.map((f) => f.key));
      const haveGroups = new Set(schema.groups.map((g) => g.id));
      ...
      commit(
        {
          groups: [...schema.groups, ...missingGroups],
          fields: [...schema.fields, ...toAdd],
        },
        `Import ${toAdd.length} data point${toAdd.length === 1 ? "" : "s"}`,
      );
    };
    ...
    const updateField: SettingsCtx["updateField"] = (key, patch) => {
      ...
      commit(
        ...
        `Edit data point “${target?.label ?? key}”`,
      );
    };
    ...
    const deleteField: SettingsCtx["deleteField"] = (key) => {
      ...
      commit(
        ...
        `Delete data point “${target?.label ?? key}”`,
      );
    };
    ...
    const moveField: SettingsCtx["moveField"] = (dragKey, overKey) => {
      ...
      commit({ ...schema, fields }, `Reorder data point “${moved.label}”`);
    };

    const addGroup: SettingsCtx["addGroup"] = (label) => {
      const name = label?.trim() || "New column";
      const taken = new Set(schema.groups.map((g) => g.id));
      const newId = uniqueKey(slugify(name) || "column", taken);
      commit(
        { ...schema, groups: [...schema.groups, { id: newId, label: name }] },
        `Add column “${name}”`,
      );
      return newId;
    };
    ...
    const renameGroup: SettingsCtx["renameGroup"] = (id, label) => {
      commit(
        {
          ...schema,
          groups: schema.groups.map((g) => (g.id === id ? { ...g, label } : g)),
        },
        `Rename column “${label}”`,
      );
    };
    ...
    const deleteGroup: SettingsCtx["deleteGroup"] = (id) => {
      const target = schema.groups.find((g) => g.id === id);
      commit(
        {
          groups: schema.groups.filter((g) => g.id !== id),
          // Drop the column's data points too.
          fields: schema.fields.filter((f) => f.group !== id),
        },
        `Delete column “${target?.label ?? id}”`,
      );
    };

    const resetSchema: SettingsCtx["resetSchema"] = () =>
      commit(cloneDefaultSchema(), "Reset schema to defaults");
